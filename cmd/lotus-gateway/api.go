...
type gatewayDepsAPI interface {
	...
	StateVMCirculatingSupplyInternalByHigh(context.Context, types.TipSetKey) (api.CirculatingSupply, error)
...
func (a *GatewayAPI) StateVMCirculatingSupplyInternalByHigh(ctx context.Context, tsk types.TipSetKey) (api.CirculatingSupply, error) {
	if err := a.checkTipsetKey(ctx, tsk); err != nil {
		return api.CirculatingSupply{}, err
	}
	return a.api.StateVMCirculatingSupplyInternalByHigh(ctx, tsk)
}
